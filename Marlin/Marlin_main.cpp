...
#include "SoftReset.h"
#define LED_PIN 4
OneButton button(5,INPUT_PULLUP);
void longpress();
void doubleclick();
...
//Fab-X 2.0
//pinMode(0,OUTPUT); //debug to led 0
pinMode(LED_PIN,OUTPUT); //debug to led 45
...
void loop()
{
  ...
  digitalWrite(11,HIGH); // extruder fan
   if(buflen < (BUFSIZE-1))
    get_command();
  #ifdef SDSUPPORT
...
void longpress()
{
 if (card.sdstop)
 {
   ...
   card.sdpause=false;
   SERIAL_ECHO_START;
   SERIAL_PROTOCOLLNPGM(MSG_STOP);
   card.sdstop=false;
  st_synchronize();
 //delay(2000);
 if(current_position[Z_AXIS] < 90 )
  {
  plan_buffer_line(max_pos[X_AXIS],max_pos[Y_AXIS],current_position[Z_AXIS] +9, current_position[E_AXIS], feedrate*feedmultiply/60/100.0, active_extruder);
  } 
//  soft_restart();
   delay(1500);
   resetFunc();
...
/*void longpress()
{
  //destination[Z_AXIS] = current_position[Z_AXIS] + 9;
//destination[E_AXIS] = current_position[E_AXIS] - 2;
//relative_mode = false;
//enable_endstops(true);
//homeaxis(X_AXIS);
 // relative_mode = false;
  /* current_position[Z_AXIS] = current_position[Z_AXIS] + 10;
 current_position[Y_AXIS] = max_pos[Y_AXIS];
 current_position[X_AXIS] = max_pos[X_AXIS];
  //attachInterrupt(1,sett,HIGH);
 // lastp[E_AXIS] = current_position[E_AXIS];
  //current_position[E_AXIS] = current_position[E_AXIS] - 2;
  //current_position[E_AXIS] = lastp[E_AXIS];
 if (card.sdstop)
 ...
 //get_coordinates();
  ...
  homeaxis(X_AXIS);
  //destination[E_AXIS] = current_position[E_AXIS] -2;
  //destination[Y_AXIS] = max_pos[Y_AXIS];
  //destination[X_AXIS] = max_pos[X_AXIS];
  //destination[X_AXIS] = 90.0 + (axis_relative_modes[X_AXIS] || relative_mode)*current_position[X_AXIS];
 // destination[Y_AXIS] = 90.0 + (axis_relative_modes[Y_AXIS] || relative_mode)*current_position[Y_AXIS];
// plan_buffer_line(max_pos[X_AXIS], max_pos[Y_AXIS], current_position[Z_AXIS] + 9, current_position[E_AXIS], feedrate*feedmultiply/60/100.0, active_extruder);
//plan_set_position(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], current_position[E_AXIS]);
  //cli()
  //sei();
  //Serial.println(destination[Y_AXIS]);
...
/*void doubleclick()
{
  if (card.sdpause==false && card.sdprinting==true )
  {
  digitalWrite(0,LOW);
//  SERIAL_PROTOCOL (card.sdpos);
  card.pauseSDPrint();
  delay(1000);
  digitalWrite(0,HIGH);
  }
  else if (card.sdprinting==false && card.sdpause==true)
 {
  digitalWrite(0,LOW);
  card.resumeSDPrint();
  //SERIAL_ECHO_START;
  //SERIAL_PROTOCOLLNPGM(MSG_RESUME);
  //SERIAL_PROTOCOL (card.sdpos);
  // card.getStatus();
  delay(1000);
  digitalWrite(0,HIGH);                          
